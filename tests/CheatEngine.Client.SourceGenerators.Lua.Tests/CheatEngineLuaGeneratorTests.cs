...
public sealed class CheatEngineLuaGeneratorTests
{
	...
	public void MapperRejectsForbiddenTypesNestedInArraysGenericsTuplesAndDtos()
	{
		...
		[
			...
			CreateMappedOperationSource("ConstrainedSnapshot<IUnsafeConstraint>",
				"internal interface IUnsafeConstraint { global::CheatEngine.SDK.Lua.References.LuaRef Reference { get; } }" + Environment.NewLine +
		...
		foreach (string source in sources)
		{
			...
			Diagnostic diagnostic = Assert.Single(run.Diagnostics.Where(static candidate => candidate.Id == "CECLUA1106"));
	...
	public void RecursiveBoundaryDiagnosticsAreDeterministic()
	{
		...
		Assert.Equal(first.GetMessage(System.Globalization.CultureInfo.InvariantCulture),
			second.GetMessage(System.Globalization.CultureInfo.InvariantCulture));
		Assert.Contains("forbidden SDK lifetime or interop type",
			first.GetMessage(System.Globalization.CultureInfo.InvariantCulture), StringComparison.Ordinal);
	}
		...
		string sourceType = "SdkSnapshot")
	{
		return string.Join(
			...
			[
				...
				"namespace TestPlugin;",
				"internal sealed class SdkSnapshot { }",
				additionalDeclarations ?? string.Empty,
				...
				"\t[CheatEngineLuaOperation(typeof(UnsafeMapper))]",
				"\t[LuaGlobal(\"unsafe\")]",
				"\tpublic static partial " + sourceType + " GetUnsafe();",
				"}"
			]);
	}
