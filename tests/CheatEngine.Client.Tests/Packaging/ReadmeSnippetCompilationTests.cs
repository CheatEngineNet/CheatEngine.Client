...
///     (<see cref="ReadmeCodeBlocks" />). Every <c>PackageReference</c> a README writes names the version this
///     project compiles against: the SDK pin, the Microsoft.Extensions.Configuration.Json version the packed template
///     stamps, and the <c>X.Y.Z</c> placeholder for CheatEngine.Client, which the reader replaces.
/// </summary>
[Collection(PackageConsumptionSmokeSerialGroup.Name)]
[Trait("Category", "PackageConsumption")]
public sealed partial class ReadmeSnippetCompilationTests(PackagedClientFeedFixture fixture)
{
	...
	/// <summary>The placeholder a README writes for the CheatEngine.Client version the reader installs.</summary>
	private const string ClientVersionPlaceholder = "X.Y.Z";
	...
	[Theory]
	[MemberData(nameof(Readmes))]
	public void EveryDocumentedPackageReferenceNamesTheCompiledVersion(string readme)
	{
		fixture.RequirePackages();
		Dictionary<string, string> expected = ExpectedReferenceVersions();
		List<DocumentedReference> references = DocumentedReferences(ReadReadme(readme));
		PackagedClientFeedFixture.Evidence(nameof(EveryDocumentedPackageReferenceNamesTheCompiledVersion),
			$"readme={readme} references=" +
			string.Join(',', references.Select(static reference => $"{reference.Id}@{reference.Version}")));
		List<string> offenders = [];
		foreach (DocumentedReference reference in references)
		{
			if (!expected.TryGetValue(reference.Id, out string? version))
			{
				offenders.Add($"line {reference.Line}: {reference.Id} is not a reference of the documented plugin " +
							  $"project ({string.Join(", ", expected.Keys)})");
			}
			else if (!string.Equals(reference.Version, version, StringComparison.Ordinal))
			{
				offenders.Add($"line {reference.Line}: {reference.Id} Version=\"{reference.Version ?? "<none>"}\", " +
							  $"expected \"{version}\"");
			}
		}

		Assert.True(offenders.Count == 0,
			$"A PackageReference of the {readme} README does not name the version its snippets compile against:" +
			Environment.NewLine + string.Join(Environment.NewLine, offenders));
	}

	[Fact]
	public void TheUmbrellaReadmeDocumentsEveryPluginProjectReference()
	{
		fixture.RequirePackages();
		string[] documented =
		[
			.. DocumentedReferences(ReadReadme(PackagedClientFeedFixture.ClientPackageId))
				.Select(static reference => reference.Id).Distinct(StringComparer.OrdinalIgnoreCase)
				.Order(StringComparer.Ordinal)
		];

		Assert.Equal(ExpectedReferenceVersions().Keys.Order(StringComparer.Ordinal), documented);
	}

	/// <summary>The <c>PackageReference</c> elements that a README writes, with their 1-based lines.</summary>
	private static List<DocumentedReference> DocumentedReferences(string markdown)
	{
		List<DocumentedReference> references = [];
		foreach (Match element in PackageReferenceElement().Matches(markdown))
		{
			Dictionary<string, string> attributes = new(StringComparer.Ordinal);
			foreach (Match attribute in XmlAttribute().Matches(element.Groups["attributes"].Value))
			{
				attributes[attribute.Groups["name"].Value] = attribute.Groups["value"].Value;
			}

			int line = markdown.AsSpan(0, element.Index).Count('\n') + 1;
			references.Add(new DocumentedReference(attributes.GetValueOrDefault("Include", string.Empty),
				attributes.GetValueOrDefault("Version"), line));
		}

		return references;
	}

	/// <summary>The version each reference of the documented plugin project must name, by package id.</summary>
	private Dictionary<string, string> ExpectedReferenceVersions()
	{
		return new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
		{
			[PackagedClientFeedFixture.ClientPackageId] = ClientVersionPlaceholder,
			[PackagedClientFeedFixture.SdkPackageId] = fixture.SdkVersion,
			[ConfigurationJsonPackageId] = PackedTemplateVersion(ConfigurationJsonPackageId)
		};
	}

	/// <summary>The version that the packed template's project gives to <paramref name="packageId" />.</summary>
	private string PackedTemplateVersion(string packageId)
	{
		...
		return (string?) template.Descendants("PackageReference")
				   .Single(reference => (string?) reference.Attribute("Include") == packageId)
				   .Attribute("Version")
			   ?? throw new InvalidOperationException($"The packed template does not version {packageId}.");
	}
	...
	private string CreateProject(bool declaresPlugin)
	{
		...
		string configurationJson = PackedTemplateVersion(ConfigurationJsonPackageId);
	...
	/// <summary>A <c>PackageReference</c> start or empty element, whatever the order of its attributes.</summary>
	[GeneratedRegex(@"<PackageReference\b(?<attributes>[^>]*)>", RegexOptions.CultureInvariant,
		RegexTimeoutMilliseconds)]
	private static partial Regex PackageReferenceElement();

	[GeneratedRegex(@"\b(?<name>[A-Za-z]+)\s*=\s*""(?<value>[^""]*)""", RegexOptions.CultureInvariant,
		RegexTimeoutMilliseconds)]
	private static partial Regex XmlAttribute();

	/// <summary>One <c>PackageReference</c> that a README writes.</summary>
	/// <param name="Id">Its <c>Include</c> attribute.</param>
	/// <param name="Version">Its <c>Version</c> attribute, or <see langword="null" /> without one.</param>
	/// <param name="Line">The 1-based README line of the element.</param>
	private sealed record DocumentedReference(string Id, string? Version, int Line);
}
