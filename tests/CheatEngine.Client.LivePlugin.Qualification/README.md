...
- **Client API only** (ADR-01). Every Cheat Engine interaction goes through `ICheatEngineClient` and its
  `IPatternScanner` (`ScanDetailed`). Cheat Engine-level setup (opening the target, allocating the scratch
