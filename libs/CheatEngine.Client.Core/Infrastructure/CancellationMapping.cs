...
/// <remarks>
///     <para>
///         A <see cref="CancellationToken" /> never interrupts a Cheat Engine call. Core observes it at three kinds of
...
///         <item>
///             <description>
///                 between two native calls of one operation, after the first began work that the second would have
///                 completed (a value scan that Cheat Engine started and the Client has not waited for yet):
///                 <see cref="CheatEngineFailureKind.Cancelled" /> with <see cref="CheatEngineHostEffect.Started" />,
///                 because that work may still be running. The caller says in the message what remains and how it ends.
///             </description>
///         </item>
///     </list>
/// </remarks>
internal static class CancellationMapping
{
	...
	/// <summary>The default message of a cancellation observed between two Cheat Engine calls of one operation.</summary>
	internal const string BetweenNativeCallsMessage =
		"The operation was cancelled after Cheat Engine began its work and before the Client completed it; no result " +
		"was published.";
	...
	/// <summary>
	///     Creates the failure of a cancellation observed after one Cheat Engine call of the operation began work and
	///     before the call that would complete it.
	/// </summary>
	/// <param name="operation">The public Client operation name.</param>
	/// <param name="message">The diagnostic message, which says what remains and how it ends.</param>
	/// <returns>
	///     A <see cref="CheatEngineFailureKind.Cancelled" /> failure whose effect is
	///     <see cref="CheatEngineHostEffect.Started" />.
	/// </returns>
	/// <remarks>The work the first call began may still be running in Cheat Engine; nothing is published.</remarks>
	internal static CheatEngineFailure BetweenNativeCalls(string operation, string message = BetweenNativeCallsMessage)
	{
		return new CheatEngineFailure(CheatEngineFailureKind.Cancelled, operation, message, null,
