...
`ClientLuaGlobals` binding generated by the CheatEngine.SDK 2.0.0 Lua binding generator and a
registered ADR-01 exception whose removal condition the architecture ratchet records. Every observation reads the opened process identifier first and again
...
as the same exception type (`LuaException`), so a failed probe is recorded as faulted evidence
