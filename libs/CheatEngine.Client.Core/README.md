...
commit and NuGet content hash, as `AssemblyMetadata`) with the informational version of the
`CheatEngine.SDK.Engine` assembly actually loaded; it reads assembly attributes only. UI/forms,
...
observed while the target changed are discarded (`Unknown` in the runtime snapshot,
`IndeterminateHostResult` from `IProcessClient`) instead of being mixed. The
