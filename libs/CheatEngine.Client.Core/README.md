...
leases on Cheat Engine's main thread before the SDK detaches. The experimental target allocations
(`CECLIENT5002`) are leases over CheatEngine.SDK's `TargetMemoryAllocator` regions, released the same
way and never through another target. They are tested against ports
...
no child-count getter and `TryGetChild` cannot tell a missing child from a failed read. `TryGetScript` cannot tell a
record without a script from a failed read either, so `Content.Script` is `null` in both cases and a failed read never
fails the snapshot; a getter that tells them apart is awaited from the SDK, like the child-count getter. A failed `Create` is rolled back once through `AddressListMutations.Delete`.
